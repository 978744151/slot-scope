<template>
    <div>
        ...
        <div class="draw_content">
            ...
            <div class="and_draw_button">
                <button @click="draw_pay_buttom" ref="rettrue">立即提现</button>
            </div>
        </div>
...
export default {
    ...
    mounted() {
        ...
        this.adminaffirm = this.$route.query.adminaffirm
        if(this.adminaffirm == '3'){
            this.$refs.rettrue.setAttribute('disabled','disabled')
            Toast({
                message: "暂时不能提现",
                position: 'bottom',
                duration: 2000
        ...
        axios.get('http://yue.wojianwang.com/api/api-bin/ywms/yuandou/data/getLastestYandou.action',
            { params: date })
            .then(res => {
                console.log(res.data)
                if (res.data.success = 1) {
